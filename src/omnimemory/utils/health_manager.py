...
from ..models.foundation.model_health_metadata import ModelHealthCheckMetadata
...
def _get_package_version() -> str:
    ...
    try:
        ...
        try:
            ...
            return str(pkg_resources.get_distribution("omnimemory").version)
        except Exception:
...
class HealthCheckConfig(BaseModel):
    ...
    metadata: ModelHealthCheckMetadata = Field(default_factory=ModelHealthCheckMetadata)
...
async def create_memory_cache_health_check() -> (
    Callable[[], Awaitable[HealthCheckResult]]
):
    """Create a memory cache health check function using ModelCachingSubcontract."""

    async def check_memory_cache() -> HealthCheckResult:
        from ..models.service.cache_subcontract import get_memory_cache

        config = HealthCheckConfig(
            name="memory_cache", dependency_type=DependencyType.CACHE
        )
        ...
        try:
            ...
            if is_healthy:
                ...
                return HealthCheckResult(
                    ...
                    status=HealthStatus.UNHEALTHY,
                    latency_ms=0.0,
                    error_message="Memory cache ping failed",
                )
        except Exception as e:
            return HealthCheckResult(
                config=config,
    ...
    async def check_pinecone() -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                status=HealthStatus.HEALTHY,
                latency_ms=0.0,
                metadata=ModelHealthCheckMetadata(),
            )
        except Exception as e:
            return HealthCheckResult(
                config=config,
